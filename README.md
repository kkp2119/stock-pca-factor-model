...
- **PC1 is a pure market factor.** It loads positively on all 20 stocks and correlates 0.958 with the Fama-French market factor (adjusted R-squared = 0.946-0.948).
...
- **Statistical elegance does not equal better portfolios.** The PCA-based tangency portfolio looks best in-sample (Sharpe 1.12) but is the worst out-of-sample performer, with the deepest drawdown (-49.5%). The naive equal-weight portfolio wins out-of-sample.
...
- **Returns:** log-returns, standardized before PCA.
- **PCA:** eigendecomposition of the standardized-return correlation matrix; explained variance ratio of component k is its eigenvalue divided by the sum of all eigenvalues.
...
<img src="assets/fig1_rolling_corr.png" width="800">
...
The first five principal components explain approximately 70.4% of total variance (41.9% / 12.8% / 7.8% / 4.2% / 3.7% for PC1-PC5), and the scree plot flattens sharply after that.

<img src="assets/fig3_scree.png" width="600">

### 2. The components have clean economic readings

<img src="assets/fig2_pc_loadings.png" width="800">
...
Regressing Apple's daily returns on PC1 and PC2 alone gives R-squared = 0.629 (t-stats of 59.1 and 27.7, F = 2,127) - evidence the factors carry explanatory power at the individual-stock level, not just in aggregate.

### 3. Fama-French validation confirms the interpretation

| PC | FF3 Adj. R-sq | FF5 Adj. R-sq | Delta |
|-----|:-----------:|:-----------:|:--------------:|
...
PC1 is almost entirely the market factor (t = 205.6 on Mkt-RF). PC2 is explained by HML (t = -67.4, raw correlation -0.804). PC3 sees the largest jump from FF3 to FF5, picking up RMW (t = 14.1) and CMA (t = 10.0) - a profitability/investment signature. PC4 and PC5 (R-squared < 0.04) look like residual, idiosyncratic noise rather than systematic risk.

<img src="assets/fig4_corr_matrix.png" width="800">

*Pairwise return correlation matrix - within-sector clustering is visible for financials (rho > 0.80) and technology.*
...
<img src="assets/fig6_tangency_weights.png" width="800">
...
<img src="assets/fig8_oos_cum_return.png" width="800">

<img src="assets/fig9_oos_drawdown.png" width="800">

<img src="assets/fig7_rolling_sharpe.png" width="800">
...
<img src="assets/fig10_efficient_frontier.png" width="700">
...
<img src="assets/fig5_drawdown_eq.png" width="800">
...
README.md
requirements.txt
Final_Proj_Code.ipynb      (full analysis: EDA to PCA to regression to FF validation to optimization to backtest)
STATS_5261_Project.pdf     (write-up this README is based on)
